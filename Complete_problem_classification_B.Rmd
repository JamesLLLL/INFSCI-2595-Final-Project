...
subtitle: "Part iv: Complete problem classification – ivB)"
author: "Jianwei Liu"
date: "2021/11/6"
...
```{r, resample}
set.seed(2021)
cv_folds <- vfold_cv(ready_v_A, v = 5, repeats = 3)
...
bp_additive <- recipe(outcome ~ ., data = ready_v_A) %>% 
...
```{r, vis_pre}
bp_additive %>% 
  prep(training = ready_v_A, retain = TRUE) %>% 
  bake(new_data = NULL) %>% 
...
```{r, show_features}
model.matrix(outcome ~ (.)^2, ready_v_A) %>% dim()
```
...
The best tuning parameter set consists of a mixing fraction equal to 0.55.
...
```{r, define_rsm3_small}
bp_rsm3_small <- recipe(outcome ~ v01 + v02 + v04 + v06 + v08 + v10 + v12, data = ready_v_A) %>% 
  step_center(all_predictors()) %>% 
  step_scale(all_predictors()) %>%
...
final_nn_model <- final_nn_fit <- final_nn_wflow %>%  fit(ready_v_A)
```
...
stan_add <- recipe(outcome ~ ., data = ready_v_A) 
rf_wflow <- workflow() %>% 
  add_model(rf_spec) %>% 
  add_recipe(stan_add)
```
...
Tune the model using Bayesian optimization:
```{r, tune_rf}
rf_set <-  rf_spec %>% parameters(rf_wflow) %>% finalize(ready_v_A)     
...
final_xgb_fit <- final_xgb_wflow %>%  fit(ready_v_A)
```

Display the confusion matrix:
```{r, confusion_matrix_xgb}
final_xgb_resample %>% collect_predictions(summarize = FALSE) %>% 
  group_nest(id, id2) %>% 
  mutate(conf_mats = map(data, 
                         ~ yardstick::conf_mat(.x, truth = outcome, estimate = .pred_class))) %>% 
  mutate(conf_table = map(conf_mats, ~as.data.frame(.x$table))) %>% 
...
set.seed(2021)
final_svm_fit <- final_svm_rbf_wflow %>%  fit(ready_v_A)
```
...
set.seed(2021)
final_knn_model <- final_knn_wflow %>%  fit(ready_v_A)
...
Based on the ROC plot, the Gradient boosted tree model still has the best performance. The second best is random forest model. Then third is neural network model.


Save the best model:
```{r, save_model}
final_xgb_fit %>% readr::write_rds("xgb_classification_v.rds")
```
