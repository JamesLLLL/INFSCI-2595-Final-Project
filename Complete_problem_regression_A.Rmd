...
subtitle: "Part iii: Complete problem regression – iiiA)"
author: "Jianwei Liu"
date: "2021/11/6"
...
### predict the continuous output with respect to the “x-variables” 
...
```{r, resample}
set.seed(2021)
cv_folds <- vfold_cv(ready_x_A, v = 5, repeats = 5)
...
bp_additive <- recipe(response ~ ., data = ready_x_A) %>% 
...
#### All pair-wise interactions model in the simplified “Getting started” portion of the project.

```{r, train_test_pairs_small}
bp_pairs_small <- recipe(response ~ x07 + x09 + x10 + x11 + x21, data = ready_x_A) %>% 
  step_center(all_predictors()) %>% 
  step_scale(all_predictors()) %>%
  step_interact( ~ all_predictors():all_predictors())

lm_pairs_small_wflow <- workflow() %>% 
  add_model(lm_spec) %>% 
  add_recipe(bp_pairs_small)

lm_pairs_small_res <- lm_pairs_small_wflow %>% 
  fit_resamples(cv_folds,
                metrics = my_metrics)

lm_pairs_small_res %>% collect_metrics()
```
...
all_cv_results <- lm_additive_res %>% collect_metrics(summarize = FALSE) %>% 
  ...
  bind_rows(lm_pairs_small_res %>% collect_metrics(summarize = FALSE) %>% 
              mutate(wflow_id = "lm_pairs_small")) 

all_cv_summaries  <- lm_additive_res %>% collect_metrics(summarize = TRUE) %>% 
  mutate(wflow_id = "lm_add") %>% 
  bind_rows(lm_pairs_res %>% collect_metrics(summarize = TRUE) %>% 
              mutate(wflow_id = "lm_pairs")) %>% 
  bind_rows(lm_pairs_small_res %>% collect_metrics(summarize = TRUE) %>% 
              mutate(wflow_id = "lm_pairs_small")) 
```
...
#### cubic RSM model in the simplified “Getting started” portion of the project

Create the cubic RSM model in the simplified “Getting started” portion of the project:

```{r, define_test_rsm3_small}
bp_rsm3_small <- recipe(response ~ x07 + x09 + x10 + x11 + x21, data = ready_x_A) %>% 
  step_center(all_predictors()) %>% 
  step_scale(all_predictors()) %>%
...
lm_rsm3_small_wflow <- workflow() %>% 
  add_model(lm_spec) %>% 
  add_recipe(bp_rsm3_small)

lm_rsm3_small_res <- lm_rsm3_small_wflow %>% 
  fit_resamples(cv_folds,
                metrics = my_metrics)

lm_rsm3_small_res %>% collect_metrics()
```
...
Fisrt define the torch version of dataset:
```{r, torch_data}
...
Split train and test with 70% and 30%:
```{r, train_test_split}
set.seed(2021)
train_indices <- sample(1:nrow(ready_x_A), round(nrow(ready_x_A)*0.7))

torch_train_ds <- torch_dataset(ready_x_A[train_indices, ])
torch_valid_ds <- torch_dataset(ready_x_A[setdiff(1:nrow(ready_x_A), train_indices), ])

length(torch_train_ds)
length(torch_valid_ds)
```

Define the data loader:
```{r, torch_dataloader}
train_dl <- torch_train_ds %>% dataloader(batch_size = 16, shuffle = TRUE)
valid_dl <- torch_valid_ds %>% dataloader(batch_size = 16, shuffle = FALSE)
valid_dl$.length()
```
...
torch_model <- torch_net()
```

Train the model. The result has some problems. so I comment it out for now:

{r, torch_train_nn}
optimizer <- optim_adam(torch_model$parameters, lr = 0.01)
...
for (epoch in 1:10) {
  ...
  cat(sprintf("MSE at epoch %d: training: %3.3f, validation: %3.3f\n", epoch, mean(train_losses), mean(valid_losses)))
} 
...
#nn_set <- nn_set %>%
#  update(hidden_units = hidden_units(c(0L, 10L)),
...
final_nn_model <- final_nn_wflow %>%  fit(ready_x_A)
```
...
Tune the model using Bayesian optimization:
```{r, tune_rf}
rf_set <-  rf_spec %>% parameters() %>% finalize(ready_x_A)
...
```{r, tune_knn}
knn_grid <- crossing(neighbors = seq(1,50),
...
From the plot we can see the neural network model performs the best base on MAE, RMSE and R-Squared.


Save the best model:
```{r, save_model}
final_nn_model %>% readr::write_rds("neural_network_regression_x.rds")
```
